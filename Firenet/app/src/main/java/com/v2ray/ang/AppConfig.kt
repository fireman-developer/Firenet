...
object AppConfig {
    const val ACTION_FORCE_LOGOUT = "com.v2ray.ang.ACTION_FORCE_LOGOUT"
    ...
    /**
     * When true, the app re-runs the best-location finder on every connect
     * instead of using a server the user pinned by hand.
     */
    const val PREF_AUTO_BEST_LOCATION = "pref_auto_best_location"
    ...
    /**
     * Payload URLs used by the automatic best-location finder to measure real
     * throughput through a candidate server. Both endpoints stream an
     * uncacheable body, which is what makes the measurement a bandwidth
     * measurement rather than a second latency measurement.
     */
    const val BANDWIDTH_TEST_URL = "https://speed.cloudflare.com/__down?bytes=6000000"
    const val BANDWIDTH_TEST_URL2 = "https://cachefly.cachefly.net/5mb.test"
    ...
    /**
     * Live speed reporting. The core runs in `:RunSoLibV2RayDaemon`, so the UI
     * process cannot read the Xray stats counters directly; it subscribes and
     * receives one tick per second instead.
     *
     * Payload of [MSG_SPEED_TICK] is `Pair<Long, Long>` = (uplink, downlink)
     * in bytes per second, measured on the proxy outbounds only.
     */
    const val MSG_SPEED_SUBSCRIBE = 8
    const val MSG_SPEED_UNSUBSCRIBE = 9
    const val MSG_SPEED_TICK = 81

    /**
     * Automatic best-location finder, handled by `V2RayTestService`.
     *
     * [MSG_MEASURE_BEST_PROGRESS] carries `Pair<Int, Int>` = (finished, total).
     * [MSG_MEASURE_BEST_SUCCESS] carries `Pair<String, Long>` = (guid, score),
     * with an empty guid meaning no usable server was found.
     */
    const val MSG_MEASURE_BEST = 73
    const val MSG_MEASURE_BEST_PROGRESS = 74
    const val MSG_MEASURE_BEST_SUCCESS = 75
    const val MSG_MEASURE_BEST_CANCEL = 76

    /** Kill switch state messages and broadcast action. */
    const val MSG_STATE_KILL_SWITCH_ON = 91
    const val MSG_STATE_KILL_SWITCH_OFF = 92
    const val ACTION_KILL_SWITCH_STOP = "com.v2ray.ang.action.killswitch.stop"
