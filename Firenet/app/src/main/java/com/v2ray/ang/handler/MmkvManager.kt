...
object MmkvManager {
    ...
    /**
     * کش درون-حافظه‌ای MMKV را خالی می‌کند و فایل‌ها را جمع می‌کند.
     *
     * داده روی دیسک دست‌نخورده می‌ماند؛ فقط صفحه‌های نگاشت‌شده در حافظه آزاد
     * می‌شوند و بار بعد که کلیدی خوانده شود دوباره از دیسک می‌آید. وقتی برنامه
     * در پس‌زمینه است و هیچ‌کس این داده‌ها را نمی‌خواند، نگه داشتنشان بی‌فایده است.
     */
    fun trimMemory() {
        listOf(
            mainStorage, profileFullStorage, serverRawStorage,
            serverAffStorage, subStorage, assetStorage, settingsStorage
        ).forEach { storage ->
            runCatching {
                storage.trim()
