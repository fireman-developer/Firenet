...
object V2RayServiceManager {
    ...
    /** کلید اشتراک صفحه‌ی اصلی در [SpeedMonitor]. */
    private const val UI_SPEED_LISTENER_KEY = "ui"
        ...
        override fun onReceive(ctx: Context?, intent: Intent?) {
            ...
            when (intent?.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_STOP -> {
                    ...
                    // Explicit stop -> never engage the kill switch.
                    V2RayVpnService.userInitiatedDisconnect = true
                    serviceControl.stopService()
                }

                AppConfig.MSG_STATE_RESTART -> {
                    Log.i(AppConfig.TAG, "Restart Service")
                    // Intentional restart -> the interim stop must not engage the kill switch.
                    V2RayVpnService.userInitiatedDisconnect = true
                    serviceControl.stopService()
                    Thread.sleep(500L)
                ...
                // صفحه‌ی اصلی سرعت لحظه‌ای می‌خواهد. تا وقتی که آن صفحه جلوی
                // چشم کاربر است تیک می‌فرستیم و به محض رفتن به پس‌زمینه قطع
                // می‌کنیم؛ در پس‌زمینه نوتیفیکیشن همین کار را ارزان‌تر می‌کند.
                AppConfig.MSG_SPEED_SUBSCRIBE -> {
                    if (coreController.isRunning) {
                        SpeedMonitor.subscribe(UI_SPEED_LISTENER_KEY) { sample ->
                            val svc = getService() ?: return@subscribe
                            MessageUtil.sendMsg2UI(
                                svc,
                                AppConfig.MSG_SPEED_TICK,
                                Pair(sample.proxyUp, sample.proxyDown)
                            )
                        }
                    }
                }

                AppConfig.MSG_SPEED_UNSUBSCRIBE -> {
                    SpeedMonitor.unsubscribe(UI_SPEED_LISTENER_KEY)
                }
            }
            ...
            when (intent?.action) {
                Intent.ACTION_SCREEN_OFF -> {
                    ...
                    // صفحه خاموش است، پس هیچ رابط کاربری‌ای برای به‌روزرسانی نمانده.
                    SpeedMonitor.unsubscribe(UI_SPEED_LISTENER_KEY)
                }
